<%@ Page Title="帐号管理" Language="C#" MasterPageFile="~/FullSite.Master" AutoEventWireup="true" CodeBehind="Vendors.aspx.cs" Inherits="VPC_2014_V001.VPC.Admin.Vendors" %>
    ...
    <form id="OperationManageForm" runat="server" class="form-horizontal">
        ...
        <div class="container-fluid">
            <asp:Repeater ID="Repeater1" runat="server" OnItemDataBound="Repeater1_ItemDataBound" OnItemCommand="Repeater1_ItemCommand">
                ...
                <ItemTemplate>
                    <tr>
                        <td><%# DataBinder.Eval(Container.DataItem, "iVendorId") %></td>
                        <td>
                            <a  href='/Vendor/Info?RealID=<%# DataBinder.Eval(Container.DataItem, "iUserId") %>&urlreferrer=<%=StringEncode("/Admin/Vendors")%>&operate=<%=StringEncode("view")%>' role="button"><%# DataBinder.Eval(Container.DataItem, "sVenName") %></a>
                        </td>
                        <td><%# DataBinder.Eval(Container.DataItem, "sVenClass") %></td>
                        ...
                        <td><%# DataBinder.Eval(Container.DataItem, "iVolumeSum") %></td>
                        <td><%# DataBinder.Eval(Container.DataItem, "Products") %></td>
                        <td>
                            <a href='/Vendor/OperationManageHistory.aspx?iUserId=<%# DataBinder.Eval(Container.DataItem, "iUserId") %>&urlreferrer=<%=StringEncode("/Admin/Vendors")%>&operate=<%=StringEncode("edit")%>' class="btn btn-primary" role="button">编辑</a>
                            <input type="button" runat="server" visible="false" iuserid='<%# DataBinder.Eval(Container.DataItem, "iUserId") %>' class="btn btn-danger" value="审核" id="show_hand" />
                            <asp:Button runat="server" Visible="false" OnClientClick="return check();" CommandName="op" Text="禁止" CommandArgument='<%# DataBinder.Eval(Container.DataItem, "iUserId") %>' ID="btn_del" CssClass="btn btn-danger" />
                        </td>
                    </tr>
                </ItemTemplate>
                <FooterTemplate>
