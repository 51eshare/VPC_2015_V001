...
namespace Common
{
    public class FileOperate
    {
        ...
        {
            string htmlfilename = string.Concat(HttpContext.Current.Server.MapPath("~/"), FileName, ".config");
            using (var sw = new StreamWriter(htmlfilename, false, Encoding.UTF8))
            {
                sw.Write(Content);
                sw.Flush();
            }
        ...
        {
            string temp = string.Concat(HttpContext.Current.Server.MapPath("~/"), filename, ".config");
            string str=string.Empty;
            if (File.Exists(temp))
            {
                    using (var sr = new StreamReader(temp, Encoding.UTF8))
                    {
                        str=sr.ReadToEnd();
                    }
            }
